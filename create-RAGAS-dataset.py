...
def evaluate(dataset_path, output_dir="evaluation_results"):
    ...
    for item in tqdm(data):
        ...
        top_k = reranked[:FINAL_K]  # NEW

        reranked_ids = extract_ids(top_k)
        update_metrics(metrics["rerank"], reranked_ids, relevant)

        # ========= BUILD RAGAS SAMPLE =========
        contexts = extract_texts(top_k)  # NEW

        ragas_dataset.append({           # NEW
            "question": question,
            "contexts": contexts,
            "ground_truth": ""
        })
    ...
    # ========= SAVE RAGAS DATASET =========  # NEW
    ragas_file = os.path.join(output_dir, f"ragas_dataset_{timestamp}.json")
    with open(ragas_file, "w", encoding="utf-8") as f:
        json.dump(ragas_dataset, f, indent=2, ensure_ascii=False)

    print(f"\nRAGAS dataset saved to: {ragas_file}")  # NEW
...
def extract_texts(results):  # NEW
    return [
        r.payload.get("text", "")  # ⚠️ поменяй ключ если нужно
        for r in results
    ]
